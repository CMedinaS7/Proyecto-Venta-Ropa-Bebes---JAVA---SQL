...
</aside> <div id=main role=main> <div id=title-bar> <ul id=breadcrumbs> <li><a href="principal.jsp" title=Home><span id=bc-home></span></a></li> <li class=no-hover>Personal</li> </ul> </div> <div class="shadow-bottom shadow-titlebar"></div> <div id=main-content> <div class=container_12> <div class=grid_12> <h1>Personal </h1></div> <div class=grid_12>
  <div class=block-border>
    <div class=block-header>
      <h1>Lista de todo el Personal registrado</h1>
      <span></span></div>
      
      

    <div class=block-content>
      <div class="alert note no-margin top">Nota: Las personas que no esten activas en la empresa, el codigo se mostrara de color rojo.</div>
        <%if("0".equals(resp)){%>
      <div class="alert info no-margin top"><span class=hide>x</span>Información: Se ha añadido un nuevo registro en la tabla personal de forma correcta!!!.</div>
      <%}else if("1".equals(resp)){%>
      <div class="alert info no-margin top"><span class=hide>x</span>Información: Se ha modificado los datos de un registro en la tabla personal de forma correcta!!!.</div>
	  ...
	  <%
				...
				String estado="";	
                    try {
                        ...
                        String cod="";
						String a="select p.IdPersonal,concat(p.Nombres,' ',p.Apellidos) as 'Nombre Completo',p.DNI,p.Telefono,p.Direccion,p.Email,c.Nombre as Cargo,p.activo from personal p, cargo c where p.IdCargo=c.IdCargo;";               
						ResultSet rs = stmt.executeQuery(a);
                        out.println("<table id=table-example class=table><thead><tr><th>IdPersonal</th><th>Nombre Completo</th><th>DNI</th><th>Telefono</th><th>Direccion</th><th>E-mail</th><th>Cargo</th></tr></thead><tbody>");
						
                        while (rs.next()) {

	    					out.println("<tr class=gradeA>"); 
                            cod=String.valueOf(rs.getString(1));
							estado=String.valueOf(rs.getString(8));
							if("1".equals(estado)){
							out.println("<td><a href=personal1.jsp?cod="+cod+">"+rs.getString(1)+"</a></td>");
                                                        }else{
                                                        out.println("<td><a href=personal1.jsp?cod="+cod+"><font color=#CC0033>"+rs.getString(1)+"</a></td>");
                                                        }
