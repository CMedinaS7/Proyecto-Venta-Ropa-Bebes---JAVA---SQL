...
</aside> <div id=main role=main> <div id=title-bar> <ul id=breadcrumbs> <li><a href="principal.jsp" title=Home><span id=bc-home></span></a></li> <li class=no-hover>Productos</li> </ul> </div> <div class="shadow-bottom shadow-titlebar"></div> <div id=main-content> <div class=container_12> <div class=grid_12> <h1> Productos </h1></div> <div class=grid_12>
  <div class=block-border>
    <div class=block-header>
      <h1>Lista de todos los productos</h1>
      <span></span></div>
    <div class=block-content>
    <div class="alert note no-margin top">Nota: Los productos que cuenten con stock menor a 5, el codigo se mostrara de color rojo.</div>
    <%if("0".equals(resp)){%>
      <div class="alert info no-margin top"><span class=hide>x</span>Se ha añadido un nuevo registro en la tabla personal de forma correcta!!!.</div>
      <%}else if("1".equals(resp)){%>
      <div class="alert info no-margin top"><span class=hide>x</span>Se ha modificado los datos de un registro en la tabla personal de forma correcta!!!.</div>
      ...
      <%
                    ...
                    try {
						...
						String a="select p.IdProducto,p.descripcion,m.nombre as Marca,p.Precio,p.Stock,p.Color,p.foto,c.Nombre as Categoria,sc.Nombre as Subcategoria from producto p,marca m, categoria c,subcategoria sc where p.IdMarca=m.IdMarca and p.idsubcategoria=sc.IdSubcategoria and c.IdCategoria=sc.idcategoria;";;               
						ResultSet rs = stmt.executeQuery(a);
                        out.println("<table id=table-example class=table><thead><tr><th>Codigo</th><th>Descripcion</th><th>Marca</th><th>Precio</th><th>Stock</th><th>Color</th><th>Foto</th><th>Categoria</th><th>SubCategoria</th></tr></thead><tbody>");
                        while (rs.next()) {
							cod=String.valueOf(rs.getString(1));
							stock=Integer.parseInt(rs.getString(5));
	    					out.println("<tr class=gradeA>"); 
							if(stock>=5){
							out.println("<td><a href=productos1.jsp?cod="+cod+">"+rs.getString(1)+"</a></td>");
                                                        }else{
                            out.println("<td><a href=productos1.jsp?cod="+cod+"><font color=#CC0033>"+rs.getString(1)+"</a></td>");
                                                        }
...
</div><div class="clear height-fix"></div> </div></div> </div> <footer id=footer><div class=container_12> <div class=grid_12> <div class="footer-icon align-center"><a class=top href="#top"></a></div> </div> </div></footer> </div> 
<script>window.jQuery||document.write('<script src="js/libs/jquery-1.6.2.min.js"><\/script>');</script> 
<script defer src='js/8f71c247c4dadc837fe569208a7a1dc0f7625c46.js'></script> <!--[if lt IE 7 ]><script src="//ajax.googleapis.com/ajax/libs/chrome-frame/1.0.3/CFInstall.min.js"></script> <script>window.attachEvent("onload",function(){CFInstall.check({mode:"overlay"})});</script><![endif]--> </body> 
</html>
