...
<div class=clearfix></div> </section> <nav id=nav>
  ...
  <h1>Pedidos realizados</h1></div> <div class=grid_12>
  <div class=block-border>
    <div class=block-header>
      <h1>Lista de todos los pedidos (REALIZADOS POR PERSONAS NATURALES Y JURIDICAS)</h1>
      <span></span></div>
    <div class=block-content>
        <div class="alert note no-margin top">
          <p>Nota: A continuación se muestran todos los pedidos, los pagados en color azul y los por pagar en color rojo. Si desea pagarlas haga click en el codigo del pedido realizado.</p>
</div>
          <%if("0".equals(resp)){%>
      <div class="alert info no-margin top"><span class=hide>x</span>Información: Se ha cancelado una reserva de forma correcta!!!.</div>
      ...
      <div class="alert error no-margin top"><span class=hide>x</span>Error: Ha ocurrido un error al relizar la operación, los datos no se han actualizado con exito, intentelo mas tarde!!!.</div>%>
        ...
        %><%
			...
			String a="SELECT  comprobante.IdComprobante AS 'COMPROBANTE',  comprobante.Fecha AS 'FECHA',  comprobante.IdCliente AS 'CLIENTE', sum(detalle_comprobante.Total) AS 'TOTAL',comprobante.Pagado FROM comprobante  INNER JOIN detalle_comprobante ON (comprobante.IdComprobante = detalle_comprobante.IdComprobante) GROUP BY comprobante.IdComprobante,  comprobante.Fecha,  comprobante.IdCliente;";               
			ResultSet rs = stmt.executeQuery(a);
                        out.println("<table id=table-example class=table><thead><tr><th>Comprobante</th><th>Fecha</th><th>Cliente</th><th>Total</th></tr></thead><tbody>");
                        while (rs.next()) {
                            
                        out.println("<tr class=gradeA>"); 
                		cod=String.valueOf(rs.getString(1));
						pag=Integer.parseInt(String.valueOf(rs.getString(5)));
						
						if (pag==0){
                                                out.println("<td><a href=ventas1.jsp?cod="+cod+"><font color=red>"+rs.getString(1)+"</font></a></td>");
                                                }else{
                                                out.println("<td><a href=ventas1.jsp?cod="+cod+">"+rs.getString(1)+"</a></td>");
                                                }
