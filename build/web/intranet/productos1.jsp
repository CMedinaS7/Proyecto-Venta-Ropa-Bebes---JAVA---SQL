...
<%
            ...
            String des="",talla="",color="",precio="",stock="",email="",cargo="",tag="",foto="",foto1="",foto2="";
...
<script src="js/funciones.js"></script>
 ...
 <script language="JavaScript">
    function redi()
    {
      document.formulario2.action="../validar_subcat";
    ...
    function redi1()
    {
      document.formulario3.action="../validar_subcat?c=1";
...
</aside> <div id=main role=main> <div id=title-bar> <ul id=breadcrumbs> <li><a href="principal.jsp" title=Home><span id=bc-home></span></a></li> <li class=no-hover>Productos</li> </ul> </div> <div class="shadow-bottom shadow-titlebar"></div> <div id=main-content> <div class=container_12>
<div class=grid_12>
  <h1>Formulario Productos</h1>
  <p>Este formulario permite grabar y modificar los datos de un producto.</p>
</div>
<div class=grid_12>
  ...
  <div class=grid_6>
    <div class=block-border>
      <div class=block-header>
        <h1>Grabar Datos de Producto</h1>
        <span></span></div>
      <form id=login-form class="block-content form" name="formulario3" action="../moduloProducto" method=post>
      <div class=_50>
            <p>
              <label for=cboscat>Seleccione Categoria:</label>
              <select class="required" name="cbocategoria" id="cbocategoria" onChange="javascript:redi1();">
                <%
           Aux = combo.llenarcat();
           String[] datos41=new String[2];
           for(int i=0; i<Aux.size();i++){
               datos41=(String[])Aux.elementAt(i);
%>
                <option value="<%=datos41[0] %>"><%=datos41[1] %></option>
                <%
  }
%>
              </select>
          ...
          <div class=_50>
            <p>
              <label for=cboscat>Sub-Categoria:</label>
              <select class="required" name="cboscat" id="cboscat">
                <%
           Aux = combo.llenarScat(c);
           String[] datos40=new String[2];
           for(int i=0; i<Aux.size();i++){
               datos40=(String[])Aux.elementAt(i);
%>
                <option value="<%=datos40[0] %>"><%=datos40[1] %></option>
                <%
  }
%>
              </select>
        ...
        <div class=_100>
          <p>
            ...
            <input id=txtdes name=txtdes class=required type=text value=""/>
          </p>
        </div>
          <div class=_50>
            <p>
              <label for=cbomarca>Marca:</label>
              <select class="required" name="cbomarca" id="cbomarca">
                <%
...
%>
          ...
          <div class=_50>
            <p>
              ...
              <input id=txttalla name=txttalla class="required digits" type=text value=""/>
            </p>
          </div>
          <div class=_50>
            <p>
              <label for=txtcolor>Color:</label>
              <input id=txtcolor name=txtcolor class="required" type=text value=""/>
            </p>
          </div>
          ...
          <div class=_50>
            <p>
              ...
              <input id=txtprecio name=txtprecio class="required digits" type=text value=""/>
            </p>
          </div>
          <div class=_50>
            <p>
              <label for=txtstock>Stock:</label>
              <input id=txtstock name=txtstock class="required digits" type=text value=""/>
            </p>
          </div>

          <div class=_100>
            <p>
              <label for=foto>Seleccione foto del producto:
                <input name="foto" type=file id="foto" class="required">
              </label>
            </p>
          </div>
          <div class=_100>
            <p>
              <label for=foto1>Seleccione foto alterna del producto:
                <input name="foto1" type=file id="foto1" class="required">
              </label>
            </p>
          </div>
          <div class="_100">
            <p>
            <label for=txttags>Ingrese tags separados por coma:</label>
            <input id=txttags name=txttags class="required" type=text value=""/>
          </p>
      </div>
        <div class=clear></div>
        <div class=block-actions>
          <ul class=actions-left>
            <li><a class="button red" id=reset-validate-form href="productos.jsp">Cancelar</a></li>
          </ul>
          <ul class=actions-right>
            <li>
              <input name="grabar" class="button" type="submit" value="Grabar Datos">
            </li>
          </ul>
        </div>
      </form>
  ...
  <% }
                    ...
                    try {
                        ...
                        String a="select p.IdProducto,p.Descripcion,p.Talla,p.Color,p.Precio,p.Stock,p.foto,p.tags,m.nombre from producto p, marca m where p.IdMarca=m.IdMarca and p.IdProducto='"+codigo+"';";               
                        ResultSet rs = stmt.executeQuery(a);
                        while (rs.next()) {

	    		        des=String.valueOf(rs.getString(2));
                        talla=String.valueOf(rs.getString(3));
                        color=String.valueOf(rs.getString(4));
                        precio=String.valueOf(rs.getString(5));
                        stock=String.valueOf(rs.getString(6));
                        foto=String.valueOf(rs.getString(7));
                        tag=String.valueOf(rs.getString(8));
  ...
  <div class=grid_6>
    <div class=block-border>
      <div class=block-header>
        ...
        <span></span></div>
      <form id=login-form2 name="formulario2"  class="block-content form" action="../moduloProducto" method="post">
        <div class=_100>
          <p><img src="<%=foto%>" width="90" height="78"></p>
          <p>
            <label for=txtdes>Descripción:</label>
            <input name=txtcodigo type="hidden" class=required id=txtcodigo value="<%=codigo%>"/>
            <input id=txtdes name=txtdes class=required type=text value="<%=des%>"/>
          </p>
        </div>
          <div class=_50>
            <p>
           ...
           for(int i=0; i<Aux.size();i++){
               datos3=(String[])Aux.elementAt(i);
%>
                <option value="<%=datos3[0] %>"><%=datos3[1] %></option>
                <%
  }
%>
              </select>
        ...
        <div class=_50>
            <p>
              <label for=cbomarca>Selecione Categoria:</label>
              <select class="required" name="cbocategoria" id="cbocategoria" onChange="javascript:redi();">
                <%
           Aux = combo.llenarcat();
           String[] datos41=new String[2];
           for(int i=0; i<Aux.size();i++){
...
%>
          ...
          <div class=_50>
           ...
           for(int i=0; i<Aux.size();i++){
               datos4=(String[])Aux.elementAt(i);
%>
                <option value="<%=datos4[0] %>"><%=datos4[1] %></option>
                <%
  }
%>
              </select>
          ...
          <div class=_50>
            <p>
              <label for=txttalla>Talla:</label>
              <input id=txttalla name=txttalla class="required digits" type=text value="<%=talla%>"/>
            </p>
          </div>
          <div class=_50>
            <p>
              <label for=txtcolor>Color:</label>
              <input id=txtcolor name=txtcolor class="required" type=text value="<%=color%>"/>
            </p>
          </div>
          ...
          <div class=_50>
            <p>
              <label for=txtprecio>Precio:</label>
              <input id=txtprecio name=txtprecio class="required digits" type=text value="<%=precio%>"/>
            </p>
          </div>
          <div class=_50>
            <p>
              <label for=txtstock>Stock:</label>
              <input id=txtstock name=txtstock class="required digits" type=text value="<%=stock%>"/>
            </p>
          </div>
          ...
          <div class="_100">
            <p>
            <label for=txttags>Ingrese tags separados por coma:</label>
            <input id=txttags name=txttags class="required" type=text value="<%=tag%>"/>
          </p>
      </div>
          <div class=clear></div>
        <div class=block-actions>
          <ul class=actions-left>
            <li><a class="button red" id=reset-validate-form2 href="productos.jsp">Cancelar</a></li>
          </ul>
          <ul class=actions-right>
            <li>
